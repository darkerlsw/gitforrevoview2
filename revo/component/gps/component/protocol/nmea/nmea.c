...
{
	const char *n;
	//ECOMM_STRING(UNILOG_PLA_STRING, nmea_validate, P_INFO, "%s", sentence);
	...
	if (NMEA_MAX_LENGTH < length) {
		REVO_LOGE(TAG, "%s() Error: sentence too long", __func__);
		return -1;
	}

	/* should start with $ */
	if ('$' != *sentence) {
		REVO_LOGE(TAG, "%s() Error: sentence header invalid", __func__);
		return -1;
	}

	/* should end with \r\n, or other... */
	if (NMEA_END_CHAR_2 != sentence[length - 1] || NMEA_END_CHAR_1 != sentence[length - 2]) {
		REVO_LOGE(TAG, "%s() Error: sentence's tail error", __func__);
		return -1;
	}
...
nmea_parse(char *sentence, size_t length, int check_checksum)
{
	...
	if (-1 == nmea_validate(sentence, length, check_checksum)) {
		REVO_LOGE(TAG, "%s() Error: sentence invalid", __func__);
		return (nmea_s *) NULL;
	}

	type = nmea_get_type(sentence);
	if (NMEA_UNKNOWN == type) {
		REVO_LOGE(TAG, "%s() Error: type unknown", __func__);
		return (nmea_s *) NULL;
	}
	...
	if (NULL == val_string) {
		REVO_LOGE(TAG, "%s() Error: _crop_sentence fail", __func__);
	    return (nmea_s *) NULL;
	}
	...
	if (0 == n_vals) {
		REVO_LOGE(TAG, "%s() Error: _split_string_by_comma fail", __func__);
		return (nmea_s *) NULL;
	}
	...
	if (NULL == parser) {
		REVO_LOGE(TAG, "%s() Error: nmea_get_parser_by_type fail", __func__);
		return (nmea_s *) NULL;
	}
	...
	if (NULL == parser->parser.data) {
		REVO_LOGE(TAG, "%s() Error: allocate_data fail", __func__);
		return (nmea_s *) NULL;
	}
